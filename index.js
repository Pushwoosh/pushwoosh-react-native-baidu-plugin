...
//});
//
//const Pushwoosh = require('pushwoosh-react-native-baidu-plugin');
//
//Pushwoosh.init({ "pw_appid" : "XXXX-XXXX", "secret_key" : "XXXXXXXXXXXXX", "reverse_proxy_url" : "your_url_proxy" });
//
//Pushwoosh.register(
//  (token) => {
...
class PushNotification {
	...
	//Function: init
	//Call this first thing with your Pushwoosh App ID (pw_appid parameter) and Baidu secret key for Android (secret_key parameter)
	//
	//Example:
	//(start code)
	//	//initialize Pushwoosh with secret_key: "BAIDU_SECRET_KEY", appid : "PUSHWOOSH_APP_ID". This will trigger all pending push notifications on start.
	//	Pushwoosh.init({ secret_key: "XXXXXXXXXXXXXXX", pw_appid : "XXXXX-XXXXX" });
	//(end)
